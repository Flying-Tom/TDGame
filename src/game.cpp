    ...
    , BGMplayer(this)
{
    ...
    scene.setSceneRect(0, 0, Map::width + 320, Map::height);
    setFixedSize(Map::width + 320, Map::height);
    ...
    enemyLambda.emplace_back([&](Enemy*& enemy) { enemy = new Dragon(this, map.flyingPath); });
    enemyLambda.emplace_back(
        [&](Enemy*& enemy) { enemy = new RobotSoldier(this, map.walkingPath); });
    enemyLambda.emplace_back([&](Enemy*& enemy) { enemy = new Shaman(this, map.walkingPath); });
    enemyLambda.emplace_back([&](Enemy*& enemy) { enemy = new Spirit(this, map.walkingPath); });
    enemyLambda.emplace_back(
        [&](Enemy*& enemy) { enemy = new FallenWarrior(this, map.walkingPath); });
    enemyLambda.emplace_back([&](Enemy*& enemy) { enemy = new Ghost(this, map.walkingPath); });
    enemyLambda.emplace_back([&](Enemy*& enemy) {
        if (QRandomGenerator::global()->bounded(0, 15) < 15) {
            ...
            while (pathLen--) {
                //                path << Map::BlockToCoordinate({QRandomGenerator::global()->bounded(0, 16),
...
void Game::endThisGame(QString s)
{
    if (s == QString("Road Error!")) {
        QMessageBox message(QMessageBox::NoIcon, "Initializing Fail", "Road Error!");
        message.setStyleSheet("QLabel{min-width: 200px;min-height: 100px;}"
        ...
        //message.setMask(QPixmap(":/images/messagebox.png").scaled(200, 100).mask());
...
int Game::RandEnemyIndex()
{
    ...
    for (size_t i = 0; i < probRec.size(); i++) {
        if (probRec[i] <= index and index < probRec[i + 1])
            return i;
    }
    return 0;
}
