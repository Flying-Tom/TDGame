...
const QVector<int> Shop::cost = { 1000, 1000, 500, 4000, 5000, 5000, 2000, 1000 };
...
void Shop::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    ...
    if (Shop::cost[Shop::map[towername]]
        ...
        painter->drawRect(
            QRectF(0, 0, awidth, aheight * (1 - qreal(counter) / cdtime[map[towername]])));
    }
}
...
void Shop::mousePressEvent(QGraphicsSceneMouseEvent* event)
{
    ...
    if (cost[map[towername]] > qgraphicsitem_cast<Statistic*>(parentItem())->money.getCurValue()) {
...
void Shop::mouseMoveEvent(QGraphicsSceneMouseEvent* event)
{
    ...
    drag->setPixmap(QPixmap(":/images/" + towername + ".png").scaled(awidth, aheight));
