...
void CampFire::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    Q_UNUSED(option)
    Q_UNUSED(widget)
...
void CampFire::aquireTarget()
{
    ...
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++) {
            QPoint block(Map::CoordinateToBlock(QPointF(x() + ax[i], y() + ay[j])));
            if (map->IsOccupied(block) == false and map->IsRoad(block) == true) {
