...
QRectF LaserTower::boundingRect() const
{
    return QRectF(-32, -64, 64, 128);
}
...
Laser *LaserTower::getTrajectory() const
{
    return trajectory;
}
...
void LaserTower::aquireTarget()
{
    ...
    foreach (QGraphicsItem *item, colliding_items) {
        Enemy *e = (Enemy *) item;
        if (item->type() == Enemy::Type and e->getMoveType() == Enemy::WALKING
            and e->getIsDead() == false) {
            atkTarget = e;
            attack();
            return;
        }
