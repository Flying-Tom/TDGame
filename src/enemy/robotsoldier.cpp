...
RobotSoldier::RobotSoldier(Game* game, QList<QPointF>& path)
    : Enemy(game,
        path,
        50,
        GameValue<qreal>(3000, 3000),
        GameValue<qreal>(10, 10),
        GameValue<qreal>(0.25, 0.25),
        Enemy::WALKING,
        200)
    , atkSpeed(50)
{
...
void RobotSoldier::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    Q_UNUSED(option)
    Q_UNUSED(widget)
    if (isStopped == true and atkTarget.isNull() == false) {
        qreal theta = 0;
    ...
    if (isDead == true) {
        ...
        HPMeter->setRect(
            -60, 40, (qreal)HP.getCurValue() / HP.getMaxValue() * 150, 12);
    }
}
...
void RobotSoldier::attack()
{
    ...
    if (((Tower*)atkTarget.data())->HP.getCurValue() <= 0) {
        isStopped = false;
        atkSpeed = 50;
    }
