    ...
    , canAttackRange(false)
{
    ...
    setPos(Map::BlockToCoordinate(Map::CoordinateToBlock(points[0])));
}

Enemy::~Enemy()
{
    --enemyCount;
    if (atkTarget.isNull() == false and atkTarget->type() == Tower::Type) {
...
void Enemy::aquireTarget()
{
    ...
    foreach (QGraphicsItem* item, colliding_items) {
        if (item->type() == Tower::Type) {
            Tower* t = (Tower*)item;
            if ((t->getAtkType() == Tower::RANGE and canAttackRange == false) or (t->getAtkType() == Tower::MELEE and canAttackMelee == false) or (t->blockNumber.getCurValue() >= t->blockNumber.getMaxValue())) {
                continue;
            } else {
                atkTarget = t;
                break;
...
void Enemy::attack()
{
    if (atkMovie.isValid() and atkMovie.currentFrameNumber() == atkMovie.frameCount() - 1) {
